...
export default {
  ...
  methods: {
    ...
    async saveCloudData(data) {
      ...
      const content = this.encodeContent(JSON.stringify(data))
      const res = await fetch(`/app/SimpleMindMap/api/files/${encodeURIComponent(cloudPath)}`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ content }) })
      if (!res.ok) throw new Error('cloud_save_failed')
      return true
    },
    ...
    downloadCloud() {
      ...
      const path = this.$route.query.cloudPath || this.$route.query.localName || '思维导图.smm'
      a.download = (path.split('/').pop() || '思维导图').replace(/\.(json|smm)$/i, '') + '.smm'
      a.click()
