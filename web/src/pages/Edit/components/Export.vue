...
export default {
  ...
  created() {
    ...
    if (this.$route.query.autoExport === '1') {
      this.$nextTick(() => this.handleShowExport(this.$route.query.exportName, this.$route.query.exportType))
    }
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    handleShowExport(fileName, exportType) {
      if (fileName) {
        this.fileName = fileName
      }
      if (exportType && this.downTypeList.some(item => item.type === exportType)) {
